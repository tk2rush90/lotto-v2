...
export class ModalService {
  ...
  registerOutlet(outlet: ModalOutletComponent): void {
    this._logger.debug('Outlet registered', outlet.id);
  ...
  unregisterOutlet(outlet: ModalOutletComponent): void {
    ...
    this._logger.debug('Outlet unregistered', outlet.id);
  }
   ...
   */
  open<C extends Modal, D = undefined, R = any>(options: ModalOptions<C, D, R>): ModalRef<C, D, R>[] {
    const modalRefs: ModalRef<C, D, R>[] = [];
    ...
    Object.keys(this._containerRefs).forEach(id => {
      ...
      this._logger.debug('Modal created', modalRef);
    });
